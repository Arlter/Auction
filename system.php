...
if (isset($data_arr)){
    while( $row = $data_arr -> fetch_array(MYSQLI_NUM)){
        ...
        if (isset($result)){
            ...
            $seller_email_subject = "[Seller Notification] Successful Auction: ".$auction_id;
            ...
            $award_email_subject = "[Award Notification] Awarded Bidder on Auction: ".$auction_id;
            ...
            // SEND WATCHER EMAIL
            $watcher_email_subject = "[Watcher Notification] Auction: ".$auction_id." has ended";
            ...
            // SEND SELLER EMAIL
            $seller_email_address =  (mysqli_query($conn, "SELECT emailAddress FROM Account, Auction WHERE accountID=seller_accountID and auctionID=$auction_id") -> fetch_array(MYSQLI_NUM))[0];
            $seller_email_subject = "[Seller Notification] Abortive Auction: ".$auction_id;
            $seller_email_content = "I am sorry to inform you that your auction ".$auction_id. " is abortive. ".$auction_url;
            send_email($seller_email_address,$seller_email_subject,$seller_email_content);
            echo $seller_email_address;
            // SEND WATCHER EMAIL
            $watcher_email_subject = "[Watcher Notification] The auction ".$auction_id." has ended";
            $watcher_email_content = "The auction ".$auction_id. " has ended. But the auction is abortive. ".$auction_url;
            $watcher_email_address =  mysqli_query($conn, "SELECT emailAddress FROM Account, BuyerWatchAuction WHERE accountID=buyer_accountID and auction_auctionID=$auction_id"); 
            if (isset($watcher_email_address)) {
                while( $watcher_email = $watcher_email_address -> fetch_array(MYSQLI_NUM)){
